...
class ResNet50(nn.Module):
    def __init__(self, num_classes=6, use_pretrained=True, freeze_base=True, dropout_rate=0.3):
        ...
        Args:
            num_classes (int): Số lượng lớp đầu ra (multi-label).
            use_pretrained (bool): Có sử dụng trọng số ImageNet hay không.
            freeze_base (bool): Nếu True thì đóng băng các tầng backbone ResNet.
        ...
        # Nếu muốn freeze backbone
        if freeze_base:
            for param in self.base_model.parameters():
                param.requires_grad = False

        # Lấy số đầu vào fc layer cuối cùng
        in_features = self.base_model.fc.in_features

        # Thay thế fc layer bằng head classifier cho multi-label
        self.base_model.fc = nn.Sequential(
            nn.Linear(in_features, 512),
            nn.ReLU(),
            nn.Dropout(p=dropout_rate),
            nn.Linear(512, num_classes),
            nn.Sigmoid()  # sigmoid để output phù hợp multi-label
        )
